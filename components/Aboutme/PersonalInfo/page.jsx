...
const PersonalInfo = () => {
    ...
    return (
        <>
            <div className="text-base mt-4 text-justify mb-4 ">
                <p>
            ...
            <div className='grid gap-3 px-4 '>
                {info.map((val, index) => (
                    <div key={index} className='flex flex-col sm:flex-row sm:items-center'>
                        <div className='min-w-[190px] text-green-500 font-extrabold text-start  text-xl italic capitalize sm:w-1'>
                            {val.title}:
                        </div>
                        <span className='text-muted font-extrabold text-md sm:w-2/3'>
                            {val.content}
                        </span>
                    </div>
                ))}
